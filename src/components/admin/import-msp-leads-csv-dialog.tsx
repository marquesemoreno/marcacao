...
import { createMspLeadsFromCsv } from "@/actions/msp-leads";
import { parseMspLeadCsv, type ParsedMspLeadRow } from "@/lib/msp-lead-csv";

/** Importação em lote de leads de MSP via CSV — pra colar direto os CSVs já
 * minerados (ver TIVDC/MSP/leads/*.csv) em vez de cadastrar um por um. */
export function ImportMspLeadsCsvDialog() {
  const [open, setOpen] = useState(false);
  const [csvText, setCsvText] = useState("");
  const [rows, setRows] = useState<ParsedMspLeadRow[]>([]);
  ...
  async function handleImport() {
    ...
    setImporting(true);
    try {
      const result = await createMspLeadsFromCsv(rows);
      if (result.created > 0) {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-2xl">
        <DialogHeader>
          <DialogTitle>Importar leads de MSP via CSV</DialogTitle>
          <DialogDescription>
            Cabeçalho com as colunas: nome (ou empresa) e telefone são obrigatórias. Segmento,
            cidade, canal e observações são opcionais. Aceita vírgula, ponto-e-vírgula ou colar
            direto de uma planilha.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-2">
          ...
          <Textarea
            ...
            placeholder={"nome,segmento,telefone,canal\nClínica Exemplo,clínica,77999998888,indicação"}
