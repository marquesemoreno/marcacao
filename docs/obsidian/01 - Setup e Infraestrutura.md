...
> `paciente@teste.com.br` autentica normalmente (a role `PATIENT` não é bloqueada em lugar nenhum), mas `/entrar` só tem redirecionamento dedicado para `ADMIN` (`/admin`) e `CLINIC` (`/clinic`) — qualquer outro papel cai em `/`. O agendamento público continua sendo guest-checkout (sem exigir login), então essa conta existe só porque foi pedida explicitamente; não há hoje uma "área do paciente" que ela desbloqueie.
